...
struct relocrecord
  {
  uint16_t addr;                  /* address that needs relocation     */
...
struct oprecord *optable = optable09;   /* used op table                     */
                                        /* size of this table                */
int16_t optablesize = sizeof(optable09) / sizeof(optable09[0]);
...
int16_t bitregtablesize = sizeof(bitregtable09) / sizeof(bitregtable09[0]);
...
uint16_t loccounter,oldlc;        /* Location counter                  */
int16_t phase;                              /* phase (offfset to ORG)            */
...
int16_t maxidlen = MAXIDLEN;                /* maximum ID length                 */
...
int16_t codeptr;                            /* byte offset within instruction    */
int16_t suppress;                           /* 0=no suppress                     */
                                        /* 1=until ENDIF                     */
                                        /* 2=until ELSE                      */
                                        /* 3=until ENDM                      */
char condline = 0;                      /* flag whether on conditional line  */
int16_t ifcount;                            /* count of nested IFs within        */
                                        /* suppressed text                   */
...
int16_t outmode = OUT_BIN;                  /* default to binary output          */
...
int16_t nRepNext = 0;                       /* # repetitions for REP pseudo-op   */
int16_t nSkipCount = 0;                     /* # lines to skip                   */

uint16_t tfradr = 0;
int16_t tfradrset = 0;

int16_t nCurLine = 0;                       /* current output line on page       */
int16_t nCurCol = 0;                        /* current output column on line     */
int16_t nCurPage = 0;                       /* current page #                    */
int16_t nLinesPerPage = 66;                 /* # lines on a page                 */
int16_t nColsPerLine = 80;                  /* # columns per line                */
...
int16_t nMacros = 0;                        /* # parsed macros                   */
int16_t inMacro = 0;                        /* flag whether in macro definition  */
int16_t lvlMacro = 0;                       /* current macro expansion level     */

char *texts[MAXTEXTS];                  /* pointers to the texts             */
int16_t nPredefinedTexts = 0;               /* # predefined texts                */
int16_t nTexts = 0;                         /* # currently defined texts         */
...
struct symrecord * findlocal(struct symrecord *sym, char forward, int16_t insert)
{
...
struct symrecord * findsym (char * nm, int16_t insert)
{
int16_t lo,hi,i,j,s;
...
char * parsestring(char *srcptr, char *dest, int16_t nlen, char **nextptr)
{
char *s = srcptr;
char *d = dest;
int16_t nInString = 0;
...
int16_t settext(char *namebuf, char *text)
{
...
long scanexpr(int16_t, struct relocrecord *);
...
if (isValidNameChar(c, 1) &&
  ...
  if ((caft == 'B') || (caft == 'F'))   /* if this might be a local reference*/
    return (int16_t)scanlabel(p);         /* look it up.                       */
  switch (c)
    {
...
long scanexpr(int16_t level, struct relocrecord *pp) /* This is what you call _recursive_ descent!!!*/
{
long t, u;
    ...
    if (*srcptr == ',')
      ...
      if (opsize == 0)
        ...
        if (unknown || !certain || dpsetting == -1 ||
          (uint16_t)(operand - dpsetting * 256) >= 256)
          opsize = 3;
        else
          opsize = 2;
        }
...
struct                                  /* Header Layout                     */
  ...
  uint16_t datasize;              /* size of binary data               */
  unsigned char unknown1[4];            /* unknown data                      */
  uint16_t extsize;               /* size of External table            */
  uint16_t startaddr;             /* program start address             */
  unsigned char unknown2[2];            /* unknown data                      */
  uint16_t globalsize;            /* size of global table              */
  unsigned char unknown3[2];            /* unknown data                      */
  uint16_t namesize;              /* length of module name             */
...
if (wcommon)                            /* if writing common data            */
  ...
  {
  int16_t extrel = 0;                       /* # external relocation records     */
...
for (i = 0; i < symcounter; i++)
  ...
  if (symtable[i].cat == SYMCAT_COMMON) /* if that is a common block         */
    ...
    j = (int16_t)ftell(objfile);            /* fill last sector with zeroes      */
    while (j % 252)
      {
      fputc(0, objfile);
      j++;
...
void outbyte(unsigned char uc, int16_t off)
{
int16_t nByte = (loccounter + off) / 8;
...
if (codeptr > 0)
  putlist("%04X ", (uint16_t)(oldlc + ((dwOptions & OPTION_LPA) ? 0 : phase)));
else if (*curline->txt)
  putlist("     ");
else
  {
...
if (lp)
  ...
  if (lp->cat == SYMCAT_PUBLICUNDEF)
    ...
    lp->value = (uint16_t)(loccounter + phase);
    }
...
{
int16_t offs;
int16_t addrelocation = 0;
      
if (p)                                  /* create relocation record          */
  p->addr = (uint16_t)(loccounter + codeptr);
...
switch (mode)
  ...
  case ADRMODE_EXT :
    if (((codebuf[0] == 0x7e) ||        /* special for JMP                   */
        ...
        pass > 1)
      {
      int16_t nDiff = (int16_t)(int16_t)operand - (int16_t)(int16_t)(loccounter + phase) - 3;
  ...
  case ADRMODE_PCR :
  case ADRMODE_PIN :
    offs = (int16_t)operand - (loccounter + phase) - codeptr - 2;
...
if (mode != ADRMODE_DIR && mode != ADRMODE_EXT)
  error |= ERR_ILLEGAL_ADDR;
offs = (uint16_t)operand - (loccounter + phase) - 2;
...
void arith(int16_t co, char noimm)
{
struct relocrecord p = {0};
...
void accarith(int16_t co, char noimm, char ignore)
{
...
void darith(int16_t co, char noimm)
{
struct relocrecord p = {0};
...
void d2arith(int16_t co, char noimm)
{
struct relocrecord p = {0};
...
switch (mode)
  ...
  case ADRMODE_IMM :
    ...
    break;
  case ADRMODE_DIR :
    putword((uint16_t)(co + 0x010));
    break;
  case ADRMODE_EXT :
    putword((uint16_t)(co + 0x030));
    break;
  default:
    putword((uint16_t)(co + 0x020));
 }
doaddress(&p);
}
...
void qarith(int16_t co, char noimm)
{
struct relocrecord p = {0};
...
if (!fnms[nfnidx])                      /* if not yet done,                  */
  fnms[nfnidx] = strdup(name);          /* remember the file name            */
while ((binlen = (int16_t)fread(binlin, 1, sizeof(binlin), srcfile)) > 0)
  {
...
void pseudoop(int16_t co, struct symrecord * lp)
{
int i, j;
char c, fillc;
struct relocrecord p = {0};
...
switch (co)
    ...
    if (generating && pass == 2)
      ...
      switch (outmode)
        ...
        case OUT_BIN :                  /* binary output file                */
          j = (int16_t)(uint16_t)operand - (int16_t)loccounter;
  ...
  case PSEUDO_SETDP :                   /* SETDP [<abs page value>]          */
    ...
    if (unknown)
      error |= ERR_LABEL_UNDEF;
    if (!(operand & 255))
      operand = (uint16_t)operand >> 8;
    if ((uint16_t)operand > 255)
      operand = -1;
    if (absmode)
      dpsetting = operand;
    else
  ...
  case PSEUDO_END :                     /* END [loadaddr]                    */
    ...
    if ((curline->lvl & 0x0f) == 0)     /* only in outermost level!          */
      ...
      if (isfactorstart(*srcptr))       /* if possible transfer address      */
        {
        tfradr = (uint16_t)       /* get transfer address              */
            scanexpr(0, &p);
  ...
  case PSEUDO_PHASE :                   /* PHASE address ?                   */
    ...
    /* ... now act on operand! */
    phase = (int16_t)operand - (int16_t)loccounter;
    break;
...
struct linebuf *macskip(struct linebuf *pmac, int16_t nSkips)
{
...
int16_t nDup = 0;                           /* # repetitions for DUP             */
int16_t terminate = 0;                      /* terminate macro expansion if set  */
int16_t skipit = 0;                         /* skip this line                    */
int16_t suppress[64] = {0};                 /* internal suppression (max.64 lvl) */
int16_t ifcount = 0;                        /* internal if counter               */
int16_t curlvl = curline->lvl & LINCAT_LVLMASK;
struct relocrecord p = {0};
      ...
      *d++ = *s++;
      ...
      if (s[1] >= '0' && s[1] <= '9')
        ...
        if (lp && *namebuf &&           /* if symbol IS a text constant,     */
            (lp->cat == SYMCAT_TEXT))
          {                             /* insert its content                */
          int16_t i = 0, j = (int16_t)(d - szLine);
          char *from = texts[lp->value];
          for (; j < LINELEN && from[i]; i++, j++)
            *d++ = from[i];
          s = nc;
...
void getoptions (int16_t argc, char* argv[])
{
int16_t i, j;
char *ld;
...
#endif
    {
    for (j = 1; j < (int16_t)strlen(argv[i]); j++)
      {
...
switch (outmode)                        /* look whether object cleanup needed*/
  ...
  case OUT_REL :                        /* FLEX Relocatable                  */
    ...
    i = (int16_t)ftell(objfile);            /* fill last sector with zeroes      */
    while (i % 252)
      {
      fputc(0, objfile);
      i++;
